...
class PlotGeneration:
    ...
    @staticmethod
    def created_plot(plots, ax, nums, coord_vehicle, names, size, color=None):

        for i in range(nums):
            plots += ax.plot(coord_vehicle[0, i, 0], coord_vehicle[0, i, 1], color=f'{color}', label=f'{names} {i}', linewidth=size)
